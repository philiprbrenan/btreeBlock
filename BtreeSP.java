...
//D1 Components                                                                 // A branch or leaf in the tree
  ...
  class Node                                                                    // A branch or leaf in the tree
   {int node;                                                                   // The number of the node
    ...
    StuckSP spLeaf, spBranch;                                                   // Stucks used in this node with their base addresses set corrctly to allow addressing of the fields in the stuck
    ...
    void setStucks()                                                            // Descriptions of the stucks addressed by this node setting their base offsets
     {Leaf     = BtreeSP.this.Leaf  .at(leafBase());                            // Address the leaf stuck
      ...
      spLeaf   = BtreeSP.this.spLeaf  .at(leafBase());                          // Address the leaf stuck
      spBranch = BtreeSP.this.spBranch.at(branchBase());                        // Address the branch stuck
     }
     ...
     {Node     branch;                                                          // The branch being searched
      ...
      findFirstGreaterThanOrEqualInBranch(int Search)                           // Find the first key in the branch that is equal to or greater than the search key
       {z(); assertBranch();
        ...
        found = t.found;
        first = t.index;
        next  = t.found ? t.data : Branch.lastElement1().data;                  // Next if no key matches
        return this;
       }
...
//D2 Split                                                                      // Split nodes in half to increase the number of nodes in the tree
     ...
     {z(); assertBranch();
      ...
      if (hasLeavesForChildren())                                               // Children are leaves
       ...
       {z();
        ...
        T.elementAt();
        tr.key  = T.key;                                                        // Parent key
        tr.unshift();                                                           // Reduce key of parent of right
    ...
    boolean stealFromRight(int index)                                           // Steal from the right sibling of the indicated child if possible
     {z(); assertBranch();
      ...
      final Node                P = this;
      final StuckSP.Transaction T = P.spBranch.new Transaction();
      T.index = index+0; T.elementAt(); final int L = T.data, Lk = T.key;
      T.index = index+1; T.elementAt(); final int R = T.data, Rk = T.key;
