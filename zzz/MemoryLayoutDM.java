   ...
   {zz();
    ...
    P.new I()
     {void a()
      ...
      String v()
       {return m.name()+"[0 +: "+N+"] <= "+m.name()+"["+source.verilogAddr()+"+:"+N+"]; /* copy2 */";
       }
      String n()
...
""");
  ...
  static void test_reduce()
   {z();
    ...
    Layout.Variable  a = s.variable ("a", 8);
    MemoryLayoutDM   S = new MemoryLayoutDM(s.compile(), "source");

    Layout           t = Layout.layout();
    Layout.Variable  b = s.variable ("b", 4);
    MemoryLayoutDM   T = new MemoryLayoutDM(s.compile(), "target");

    T.copy(S.at(a));
    ok(T.P.printVerilog(), """
   1  target[0 +: 4] <= target[       0/*a       */+:4]; /* copy2 */
""");
    }
