...
my @ext     = qw(.html .java .jpg .md .pl .pdf .png .py .rpt .sdc .txt .xdc .zip); # Extensions of files to upload to github
...
if (1)                                                                          # Write workflow
 {my @j = map {fn $_}  grep {fn($_) !~ m(Able\Z)}                               # Java files to test do not include interfaces
  ...
  @j = qw(BtreeSF         BtreeBam BtreeBan Ban BtreeBap);                      # This will test all the stuff of current interest
