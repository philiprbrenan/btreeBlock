...
//D2 Split                                                                      // Split nodes in half to increase the number of nodes in the tree
  ...
  private void splitLeafRoot()                                                  // Split a leaf which happens to be a full root into two half full leaves while transforming the root leaf into a branch
   {z(); T.setIntInstruction(node_assertLeaf, root); assertLeaf();
    ...
    for (int i = 0; i < splitLeafSize; i++)                                     // Build left leaf from parent
     {z(); lT.shift();
      lL.T.at(lL.tKey ).move(lT.T.at(lT.tKey ));                                /// Parallel possible
      lL.T.at(lL.tData).move(lT.T.at(lT.tData));
      lL.push();
     }
    for (int i = 0; i < splitLeafSize; i++)                                     // Build right leaf from parent
     {z(); lT.shift();
      lR.T.at(lR.tKey ).move(lT.T.at(lT.tKey));                                 /// Parallel possible
    ...
    bT.T.at(bT.tKey ).move(T.at(flKey));                                        /// Parallel possible
  ...
  private void splitBranchRoot()                                                // Split a branch which happens to be a full root into two half full branches while retaining the current branch as the root
   {z(); T.setIntInstruction(node_assertBranch, root); assertBranch();
    ...
    bT.T.at(bT.tKey) .move(T.at(parentKey));                                    /// Parallel possible
    bT.T.at(bT.tData).move(T.at(l));
    bT.push();
    bT.T.at(bT.tKey ).zero();
    bT.T.at(bT.tData).move(T.at(r));
  ...
  private void splitLeaf()                                                      // Split a leaf which is not the root
   {z(); tt(node_assertLeaf, node_splitLeaf); assertLeaf();
    ...
    for (int i = 0; i < splitLeafSize; i++)                                     // Build left leaf
     {z(); lR.shift();
      lL.T.at(lL.tKey ).move(lR.T.at(lR.tKey ));                                /// Parallel possible
    ...
    bT.T.at(bT.tData).move(T.at(l));                                            /// Parallel possible
  ...
  private void splitBranch()                                                    // Split a branch which is not the root by splitting right to left
   {z();
    ...
    tt(node_assertBranch, node_splitBranch); assertBranch();
    ...
    for (int i = 0; i < splitBranchSize; i++)                                   // Build left branch from right
     {z(); bR.shift();
      bL.T.at(bL.tKey ).move(bR.T.at(bR.tKey ));                                /// Parallel possible
    ...
    bT.T.at(bT.tKey ).move(bR.T.at(bR.tKey));                                   /// Parallel possible
    bT.T.at(bT.tData).move(T.at(l));
    bT.T.at(bT.index).move(T.at(index));
    bT.insertElementAt();
   }
  ...
  private void stealFromLeft()                                                  // Steal from the left sibling of the indicated child if possible to give to the right - Dennis Moore, Dennis Moore, Dennis Moore.
   {z();
    P.new Block()
     {void code()
       {z();
        tt(node_assertBranch, node_stealFromLeft); assertBranch();
        P.new If (T.at(index))
         {void Else()
           {T.at(stolenOrMerged).zero();
            P.Goto(end);
        ...
        P.new If (T.at(hasLeavesForChildren))                                   // Children are leaves
         {void Then()
           {tt(node_leafBase, l); leafBase(); lL.base(T.at(leafBase));
            ...
            lR.T.at(lR.tKey ).move(lL.T.at(lL.tKey ));                          /// Parallel possible
            ...
            bT.T.at(bT.tKey) .move(lL.T.at(lL.tKey));                           /// Parallel possible
            bT.T.at(bT.tData).move(T.at(l));
            bT.T.at(bT.index).move(T.at(index));
            bT.T.at(bT.index).dec();
          ...
          void Else()                                                           // Children are branches
           {z();
            ...
            bR.T.at(bR.tKey) .move(bT.T.at(bT.tKey));                           /// Parallel possible
            ...
            bT.T.at(bT.tKey) .move(bL.T.at(bL.tKey));                           /// Parallel possible
            bT.T.at(bT.tData).move(T.at(l));
            bT.T.at(bT.index).move(T.at(index));
            bT.T.at(bT.index).dec();
            bT.setElementAt();                                                  // Reduce key of parent of left
  ...
  private void stealFromRight()                                                 // Steal from the right sibling of the indicated child if possible
   {z();
    P.new Block()
     {void code()
       {z();
        tt(node_assertBranch, node_stealFromRight); assertBranch();
        ...
        T.at(lk).move(bT.T.at(bT.tKey));                                        /// Parallel possible
        T.at(l) .move(bT.T.at(bT.tData));
        bT.T.at(bT.index).move(T.at(index));
        bT.T.at(bT.index).inc();
        bT.elementAt();

        T.at(rk).move(bT.T.at(bT.tKey));                                        /// Parallel possible
        T.at(r) .move(bT.T.at(bT.tData));
        ...
        P.new If(T.at(hasLeavesForChildren))                                    // Children are leaves
         {void Then()
           {z();
            ...
            bT.T.at(bT.tKey) .move(lR.T.at(lR.tKey));                           /// Parallel possible
            bT.T.at(bT.tData).move(T.at(l));
            bT.T.at(bT.index).move(T.at(index));
          ...
          void Else()                                                           // Children are branches
           {z();
            ...
            bL.T.at(bL.tKey).move(T.at(lk));                                    /// Parallel possible
...
//D2 Merge                                                                      // Merge two nodes together and free the resulting free node
                 ...
                 {void code()
                   {for (int i = 0; i < maxKeysPerLeaf(); ++i)                  // Merge in left child leaf
                     {tt(node_isEmpty, l);
                      ...
                      lL.shift();
                      lT.T.at(lT.tKey ).move(lL.T.at(lL.tKey));                 /// Parallel possible
                 ...
                 {void code()
                   {for (int i = 0; i < maxKeysPerLeaf(); ++i)                  // Merge in right child leaf
                     {tt(node_isEmpty, r);
                      ...
                      lR.shift();
                      lT.T.at(lT.tKey ).move(lR.T.at(lR.tKey));                 /// Parallel possible
  ...
  private void mergeLeftSibling()                                               // Merge the left sibling
   {z();
    P.new Block()
     {void code()
       {tt(node_assertBranch, node_mergeLeftSibling); assertBranch();
             ...
             {void code()
               {for (int i = 0; i < maxKeysPerBranch(); i++)                    // Transfer left to right
                 {z();
                  ...
                  branchSize();
                  bL.pop();
                  bR.T.at(bR.tKey ).move(bL.T.at(bL.tKey));                     /// Parallel possible
                  bR.T.at(bR.tData).move(bL.T.at(bL.tData));
                  bR.unshift();
     ...
     {void code()
       {final ProgramPA.Label Return = end;
        ...
        P.new If (T.at(found))                                                  // Found the key in the leaf so update it with the new data
         {void Then()
           {lT.T.at(lT.tKey ).move(T.at(Key));                                  /// Parallel possible
