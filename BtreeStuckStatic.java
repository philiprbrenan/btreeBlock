...
class BtreeStuckStatic extends Test                                             // Manipulate a btree
 {final int
...
//D1 Construction                                                               // Create a BTree from nodes which can be branches or leaves.  The data associated with the BTree is stored only in the leaves opposite the keys
  ...
  BtreeStuckStatic(int MaxKeysPerLeaf, int MaxKeysPerBranch)                    // Define a BTree with the specified dimensions
   {zz();
...
//D1 Components                                                                 // A branch or leaf in the tree
  ...
  class Node                                                                    // A branch or leaf in the tree
   {boolean isLeaf;                                                             // A leaf if true
    ...
    class KeyStuck  extends StuckStatic {int maxSize() {return maxKeysPerLeaf;}};     // Key, data pairs when a leaf
    class NextStuck extends StuckStatic {int maxSize() {return maxKeysPerBranch+1;}}; // Key, next pairs when a branch. The last entry is top and so this stack must always have at least one element in it when the node is acting as a branch

    final StuckStatic keyData = new KeyStuck();                                 // Key, data pairs when a leaf
    final StuckStatic keyNext = new NextStuck();                                // Key, next pairs when a branch. The last entry is top and so this stack must always have at least one element in it when the node is acting as a branch
    ...
    boolean hasLeavesForChildren()                                              // The node has leaves for children
     {z(); assertBranch();
      final int last = keyNext.lastElement1().data;
      return nodes.elementAt(last).isLeaf;
     }

    int top()                                                                   // The top next element of a branch
     {z(); assertBranch();
      return keyNext.elementAt1(branchSize()).data;
     }
      ...
      if (isLeaf)                                                               // Print a leaf
       {s.append("Leaf(node:"+node+" size:"+leafSize()+")\n");
        ...
        for (int i = 0; i < keyData.size(); i++)
         {final StuckStatic.ElementAt kd = keyData.elementAt1(i);
       ...
       {s.append("Branch(node:"+node+
        ...
        for (int i = 0; i < keyNext.size(); i++)
         {final StuckStatic.ElementAt kn = keyNext.elementAt1(i);
...
//D2 Array                                                                      // Represent the contents of the tree as an array
    ...
    void leafToArray(Stack<KeyData>s)                                           // Leaf as an array
     {z(); assertLeaf();
      ...
      for  (int i = 0; i < K; i++)
       {z();
        final StuckStatic.ElementAt e = keyData.elementAt1(i);
     ...
     {z(); assertBranch();
      ...
      if (K > 0)                                                                // Branch has key, next pairs
       {z();
        for  (int i = 0; i < K; i++)
         {z();
          final int next = keyNext.elementAt1(i).data;                           // Each key, next pair
...
//D2 Print                                                                      // Print the contents of the tree
    ...
    void printLeaf(Stack<StringBuilder>S, int level)                            // Print leaf horizontally
     {assertLeaf();
      ...
      final int K = leafSize();
      for  (int i = 0; i < K; i++) s.append(""+keyData.elementAt1(i).key+",");
      ...
      if (K > 0)                                                                // Branch has key, next pairs
       {for  (int i = 0; i < K; i++)
         {final int next = keyNext.elementAt1(i).data;                          // Each key, next pair
          ...
          S.elementAt(L+0).append(""+keyNext.elementAt1(i).key);                // Key
          S.elementAt(L+1).append(""+node+(i > 0 ?  "."+i : ""));               // Branch,key, next pair
          S.elementAt(L+2).append(""+keyNext.elementAt1(i).data);
         }
       }
...
//D2 Split                                                                      // Split nodes in half to increase the number of nodes in the tree
    ...
    void splitLeafRoot()                                                        // Split a leaf which happens to be a full root into two half full leaves while transforming the root leaf into a branch
     {z(); assertLeaf();
      ...
      for (int i = 0; i < splitLeafSize; i++)                                   // Build left leaf
       {z();
        final StuckStatic.Shift f = keyData.shift1();
      ...
      final int last  = l.keyData. lastElement1().key;                          // Last of left leaf
    ...
    void splitBranchRoot()                                                      // Split a branch which happens to be a full root into two half full branches while retaining the current branch as the root
     {z(); assertBranch();
      ...
      for (int i = 0; i < splitBranchSize; i++)                                 // Left
       {z();
        final StuckStatic.Shift f = keyNext.shift1();
        l.keyNext.push(f.key, f.data);
       }
      final StuckStatic.Shift pl = keyNext.shift2();                            // This key, next pair will be part of the root
      ...
      final StuckStatic.Shift pr = keyNext.shift3();                            // Top of root
    ...
    void splitLeaf(Node parent, int index)                                      // Split a leaf which is not the root
     {z(); assertLeaf();
      ...
      for (int i = 0; i < splitLeafSize; i++)                                   // Build left leaf
       {z();
        final StuckStatic.Shift f = r.keyData.shift1();
        l.keyData.push(f.key, f.data);
       }
      final int F = r.keyData.firstElement1().key;
      final int L = l.keyData.lastElement1 ().key;
    ...
    void splitBranch(Node parent, int index)                                    // Split a branch which is not the root by splitting right to left
     {z(); assertBranch();
      ...
      for (int i = 0; i < splitBranchSize; i++)                                 // Build left branch
       {z();
        final StuckStatic.Shift f = r.keyNext.shift1();
        l.keyNext.push(f.key, f.data);
       }

      final StuckStatic.Shift split = r.keyNext.shift1();                       // Build right branch
    ...
    boolean stealFromLeft(int index)                                            // Steal from the left sibling of the indicated child if possible to give to the right - Dennis Moore, Dennis Moore, Dennis Moore.
     {z(); assertBranch();
      ...
      final StuckStatic.ElementAt  L = keyNext.elementAt1(index-1);
      final StuckStatic.ElementAt  R = keyNext.elementAt2(index+0);
      final StuckStatic            p = keyNext;
      ...
      if (hasLeavesForChildren())                                               // Children are leaves
       {z();
        ...
        final StuckStatic.LastElement ll = l.lastElement1();
        r.insertElementAt(ll.key, ll.data, 0);                                  // Increase right
        l.pop();                                                                // Reduce left
        p.setElementAt   (l.elementAt1(nl-2).key, L.data, index-1);             // Swap key of parent
       }
       ...
       {z();
        ...
        final StuckStatic.LastElement  t = l.lastElement1();                    // Increase right with left top
        ...
        p.setElementAt(l.lastElement1().key,      L.data, index-1);             // Reduce key of parent of left
       }
      return true;
     }
    ...
    boolean stealFromRight(int index)                                           // Steal from the right sibling of the indicated child if possible
     {z(); assertBranch();
      ...
      final StuckStatic.ElementAt L = keyNext.elementAt1(index+0);
      final StuckStatic.ElementAt R = keyNext.elementAt2(index+1);
      final StuckStatic           p = keyNext;
      if (hasLeavesForChildren())                                               // Children are leaves
       {z();
        final StuckStatic         l = nodes.elementAt(L.data).keyData;
       ...
       {z();
        ...
        l.setElementAt(L.key, l.lastElement1().data, nl);                       // Left top becomes real
...
//D2 Merge                                                                      // Merge  two nodes together and free the resulting free node
    ...
    boolean mergeRoot()                                                         // Merge into the root
     {z();
      ...
      final Node r = nodes.elementAt(keyNext. lastElement1().data);
      ...
      if (p.hasLeavesForChildren())                                             // Leaves
       {z();
        if (l.leafSize() + r.leafSize() <= maxKeysPerLeaf)
         {z(); p.keyData.clear();
          ...
          for (int i = 0; i < nl; ++i)
           {z();
            final StuckStatic.Shift f = l.keyData.shift1();
       ...
       {z();
        ...
        for (int i = 0; i < nl; ++i)
         {z();
          final StuckStatic.Shift f = l.keyNext.shift1();
          p.keyNext.push(f.key, f.data);

         }
        p.keyNext.push(pkn.key, l.keyNext.lastElement1().data);
        ...
        p.keyNext.push(0, r.keyNext.lastElement2().data);                       // Top so ignored by search ... except last
        return true;
       }
      return false;
     }
     ...
     {z(); assertBranch();
      ...
      if (hasLeavesForChildren())                                               // Children are leaves
       ...
       {z();
        ...
        final int t = p.elementAt1(index-1).key;                                // Top key
        r.insertElementAt(t, l.lastElement1().data, 0);                         // Left top to right
        ...
        p.removeElementAt1(index-1);                                            // Reduce parent on left
      return true;
     }
      ...
      if (hasLeavesForChildren())                                               // Children are leaves
       {z();
        ...
        for (int i = 0; i < maxKeysPerLeaf && r.size() > 0; i++)                // Transfer right to left
         {z();
          final StuckStatic.Shift q = r.shift1();
          l.push(q.key, q.data);
         }
       }
      else                                                                      // Children are branches
       {z();
        ...
        final StuckStatic.LastElement ll = l.lastElement1();
        l.setElementAt(p.elementAt1(index).key, ll.data, nl);                   // Re-key left top

        for (int i = 0; i < maxKeysPerBranch && r.size() > 0; i++)              // Transfer right to left
         {z();
          final StuckStatic.Shift f = r.shift1();
          l.push(f.key, f.data);
         }
       }

      final StuckStatic.ElementAt pkn = p.elementAt1(index+1);                  // Key of right sibling
      p.setElementAt(pkn.key, p.elementAt2(index).data, index);                 // Install key of right sibling in this child
      p.removeElementAt1(index+1);                                              // Reduce parent on right
      return true;
     }
...
//D2 Balance                                                                    // Balance the tree by merging and stealing
    ...
    void balance(int index)                                                     // Augment the indexed child so it has at least two children in its body
     {z(); assertBranch();
      ...
      final StuckStatic.ElementAt p = nodes.elementAt(node).keyNext.elementAt1(index);
...
//D1 Deletion                                                                   // Delete a key, data pair from the tree
  ...
  Integer findAndDelete(int Key)                                                // Delete a key from the tree and returns its data if present without modifying the shape of tree
   {z();
    ...
    final StuckStatic.ElementAt kd = l.keyData.elementAt1(i);                   // Key, data pairs in the leaf
    l.keyData.removeElementAt1(i);                                              // Remove the key, data pair from the leaf
    return kd.data;
   }
...
//D0 Tests                                                                      // Testing
  ...
  static void test_put_ascending()
   {final BtreeStuckStatic t = new BtreeStuckStatic(4, 3);
    final int N = 64;
...
""");
  ...
  static void test_put_ascending_wide()
   {final BtreeStuckStatic t = new BtreeStuckStatic(8, 7);
    final int N = 64;
    for (int i = 1; i <= N; ++i) t.put(i);
    //stop(t);
    t.ok("""
...
""");
  ...
  static void test_put_descending()
   {final BtreeStuckStatic t = new BtreeStuckStatic(2, 3);
    final int N = 64;
...
""");
  ...
  static void test_put_small_random()
   {final BtreeStuckStatic t = new BtreeStuckStatic(6, 3);
...
""");
  ...
  static void test_find()
   {final BtreeStuckStatic t = new BtreeStuckStatic(8, 3);
    final int N = 32;
