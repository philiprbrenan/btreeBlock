...
public class Machine3                                                           // Define a machine along with its associated Instruction set, assembler and emulator
 {static boolean Trace  = false, Dump = false, LowLevelCode = false;            // Trace program execution, dump memory after execution, print low level instructions
  ...
  final Map<Integer,I> manual    = new TreeMap<>();                             // Manual describing each instruction used
  ...
  public void printProgramSizes()                                               // Print a table of program sizes
   {if (!PrintNumberOfLines) return;
    say("<h1>Program sizes</h1>");
    say("<p>The following table shows the number of instructions in each program run:");
    say("<table cellpadding=10 border=0>");
    say("<tr><td><b>Count</b><td><b>Program</b>");

    for(String t : sizes.keySet()) say("<tr><td>"+sizes.get(t)+"<td>"+t);
...
//void not         (int r)          {C.push(new I("not"               ) {void i() { R[r]     = ~R[r];           }  void lid() {boxes(22, null, r,    null, "R",  "");}});}  // Used ?
  ...
  void putRegs     ()               {C.push(new I("putRegs"           ) {void i() { sayIntsX(R);                }  void lid() {boxes(27, null, null, null, "",   "Write all the registers"                                                                   );}});}
...
// Documentation
  ...
  public void printLog()                                                        // Print the log to a file
   {printManual();                                                              // Include the manual if requested
...
// Utility functions
  ...
  public void sayIntsX(int[]a)                                                  // Say the integers in an array in hexadecimal
   {String s = "";
    for(int i = 0; i < a.length; ++i) s += " " + String.format("%4x", a[i]);
    if (s.length() > 0) say(s.substring(1, s.length()));
   }

  public static void main2(String args[])                                       // Test one program
  ...
  public static void main(String args[])                                        // Test all programs
   {final Machine3 m = new Machine3();
    Trace = true; Dump = true; LowLevelCode = true; PrintProgram = true;  PrintNumberOfLines = true;         // Options
