...
class Machine3                                                                  // Define a machine along with its associated Instruction set, assembler and emulator
 {static boolean Trace  = false, Dump = false;                                  // Trace program execution, dump memory after execution
  ...
  enum OpCodes {add, and, cmpGtrc, cmpLtrc, cmpEqrc, cmpGtrr, cmpLtrr, cmpEqrr, // Operation codes
    ...
    sl, sr, stop, strd, stri, sub, xor, label;};
  ...
  void printProgram()                                                           // Print a program
   {if (!PrintProgram || execMode != ExecMode.high) return;
    ...
    for(I i : C)                                                                // Locate the address of each instruction in the program and the length of each instruction
     {final I t = i.target;
      say(String.format
       ("<tr><td>%4d<td>%4s<td>%-12s<td>%4d<td>%4d<td>%4d",
        i.address,   //i.next,
  ...
  static void printLog()                                                        // Print the log to a file
   {try
     ...
     {final I i = C.elementAt(pc);                                              // Instruction
      ...
      if (i.stop)                                                               // Stop execution
       {if (Trace)                                                              // Print number of instructions and number of jumps executed
         {say("</pre>",
              "<p>Run result: Normal completion after executing: ",
               z, " instructions and ",
               J, " jumps in program ", Title);
         }
      ...
      pc = i.jump ? i.constantBox * 2 : pc + 1;                                 // Next instruction address or the target of a jump as taken from the constant box to prove that the constant box address works as expected rather than using the address of the target instruction
       ...
       {final OpCodes O = OpCodes.values()[i & ~OpCodeBase & 0xFF];             // Instruction to execute
        ...
        switch(O)                                                               // Decode operation code
         {case add        : R[T]     += R[S];                            break;
          ...
          case dec        : R[S]--;                                      break;
          case inc        : R[S]++;                                      break;
          case jump       : pc = ja;  jump = true;                       break;
          case jumpBckIfNz: if (R[S]  > 0)    {pc = ja;  jump = true;}   break;
          case jumpFwdIfZ : if (R[S] == 0)    {pc = ja;  jump = true;}   break;
          case label      :                                              break;
...
//void cmpLtrr     (int t, int s) {C.push(new I() {void i() {cmpLtRR(t, s);    } void lid() {boxes(OpCodes.cmpLtrr    , t, s, 0, "RR", "Compare two registers and set the first register to one if it is less than the second register else zero");}});}
  ...
  void jump        (int c)        {C.push(new I() {void i() {J(true);          } void lid() {boxes(OpCodes.jump       , 0, 0, c, "C",  "");}});}
...
//void jumpFwdIfNz (int r, int c) {C.push(new I() {void i() {J(R[r] >  0);     } void lid() {boxes(OpCodes.jumpFwdIfNz, 0, r, c, "RC", "Jump forwards to the specified location in the program if the register is not zero - useful for constructing if statements");}});}
  ...
  void xor         (int r, int c) {C.push(new I() {void i() {R[r] = R[r] ^ c;  } void lid() {boxes(OpCodes.xor        , 0, r, c, "RC", "Xor a constant with a register and save the result in the register");}});}
...
// Encode two bytes and save their parity bits
  ...
  void correct(int match, int address, int flip)                                // Correct a parity bit error given the disagreement vector in register a
   {ldrr(b, a);                                                                 // 'a' shows the points of disagreement
    ...
    cmpEqrr(b, c);
    int noMatch = label();
    jumpFwdIfZ(b, noMatch);                                                     // if match equals 'a'
  ...
  void Program2()                                                               // Program 1 as described in: CSE141L - Term Project.pdf
   {ldrc(a, 94);                                                                // Address source byte
    int loop = label();                                                         // For a..0
      ...
      ldrc(e, 1);
      and (e, d);
      strd(e, q16);                                                             // All registers now free
      ...
      ldrd(c, q16);
      cmpEqrr(b, c);
      sl  (b, 4);                                                               // Position in result mask
      or  (a, b);                                                               // Or into position

      ldrd(b, p8);                                                              // P8 Q8
      ldrd(c, q8);
      cmpEqrr(b, c);
      sl  (b, 3);                                                               // Position in result mask
      or  (a, b);                                                               // Or into position

      ldrd(b, p4);                                                              // P4 Q4
      ldrd(c, q4);
      cmpEqrr(b, c);
      sl  (b, 2);                                                               // Position in result mask
      or  (a, b);                                                               // Or into position

      ldrd(b, p2);                                                              // P2 Q2
      ldrd(c, q2);
      cmpEqrr(b, c);
      sl  (b, 1);                                                               // Position in result mask
      or  (a, b);                                                               // Or into position
      ...
      ldrc(b, 0b11111);
      and (a, b);
      jumpFwdIfZ(a, parityCheckEnd);                                            // 'a' shows the parities that do not match at: 16|8|4|2|1 . a == 0 implies that the parities match.

      correct(0b1_1111, inHigh, 0b0000_0100);                                   // 11
...
// Utility functions
  ...
  static void One()                                                             // Test one program
   {final Machine3 m = new Machine3();
    Trace = true; Dump = true; PrintProgram = false; PrintNumberOfLines = false;// Options
    m.testProgram2a(ExecMode.high);
    m.sayTests();
   }
   ...
   {final Machine3 m = new Machine3();
    ...
    for (ExecMode em : EM)
     {Trace = true;
      ...
      m.testProgram2a (em);                                                     // Program 2 from the term paper
