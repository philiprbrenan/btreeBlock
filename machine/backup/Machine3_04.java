       ...
       {final OpCodes O = OpCodes.values()[i & ~OpCodeBase & 0xFF];             // Instruction to execute
        ...
        switch(O)                                                               // Decode operation code
         {case add        : R[T]     += R[S];                            break;
          ...
          case dec        : R[S]--;                                      break;
          case inc        : R[S]++;                                      break;
          case jump       : pc = C * 4;  jump = true;                    break; // 4 low level clocks per high level instruction, but we could have made it some other number if we wished without altering the high level instruction set architecture.
          case jumpBckIfNz: if (R[S]  > 0)    {pc = C * 4;  jump = true;}break;
